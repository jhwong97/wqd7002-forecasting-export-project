...
- **Data Understanding** phase: Encompasses several processes to gain a comprehensive understanding into the chosen datasets intended for both exploratory analysis and modeling. In this research, five key processes of data understanding have been implemented: **data collection**, **data description**, **data quality assessment**, **data exploration** and **data relationships**.
...
5.  **Limitations of Statistical Models**: Models like ARIMA and SARIMA underperformed primarily due to the complexity and non-linearity data of Malaysia's total export value.
...
1. Broadening Research Scope: Future studies should aim to predict export values of specific commodities and explore other variables affecting these values.
2. Incorporating Granular Data such as daily data in the models to improve accuracy and insights.
...
2. Follow the procedures of the data processing workflow according to [this](test_code_area.ipynb). It covers the following phases:
    ...
    - **Data Modification** to conduct feature engineering process like data transformation to prepare the dataset for exploratory data analysis and modelling purposes.
    ...
    - Developed **statistical models** (**ARIMA** and **SARIMA**) and **machine learning regression models** (**Linear Regression, Support Vector Regressor, Random Forest, Gradient Boosting**) to predict Malaysia's total export value. The results of the charts plotted are stored in [here](/images/). The modelling part is carried out using python packages like `sklearn` and `pmdarima`.
...
- Uploading a copy of the raw data and transformed data to gcs.
